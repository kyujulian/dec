...
function Example({ collections, setCollection }: { collections: Collection[], setCollection: Dispatch<SetStateAction<string>> }) {
  ...
  return (
    <div className="fixed top-16 w-72">
      <Listbox value={selected} onChange={selectCollection}>
        <div className="relative mt-1">
          ...
          >
            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {collections.map((collection, personIdx) => (
                <Listbox.Option
                  key={personIdx}
                  className={({ active }) =>
